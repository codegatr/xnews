...
try {
    ...
    if ($islem === 'guncelle' && post()) {
        ...
        // manifest.json'un yeni sürümünü DB'ye yaz.
        // GUVENLIK: Manifest yanlış taglenmişse (tag != manifest surum), GitHub release
        // tag'ini kullan - DB tutarsızlığı olmasın. Manifest null olabilir.
        $yeni_manifest = manifest_oku() ?: ['surum' => $yeni_surum];
        $dogru_surum = $yeni_surum; // GitHub release tag'inden (en güvenilir kaynak)
        ayar_guncelle('mevcut_surum', $dogru_surum);

        // Manifest yanlış taglenmiş ise uyarı logla + manifest.json'u DB'ye eşitle
        if (!empty($yeni_manifest['surum']) && $yeni_manifest['surum'] !== $dogru_surum) {
            log_ekle('uyari', 'Manifest/Tag sürüm uyuşmazlığı',
                'Tag: v' . $dogru_surum . ', manifest.json: v' . $yeni_manifest['surum'] .
                '. DB tag sürümüne set edildi.', $yonetici['id']);
            // Manifest dosyasını da düzelt (gelecek sefer doğru gözüksün)
            $yeni_manifest['surum'] = $dogru_surum;
            @file_put_contents(__DIR__ . '/manifest.json',
                json_encode($yeni_manifest, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        }

        // OTOMATIK DB MIGRATION: sql/migration_v*.sql dosyasi varsa calistir
        $migration_sayisi = 0;
        $migration_yolu = __DIR__ . '/sql/migration_v' . $dogru_surum . '.sql';
        if (file_exists($migration_yolu)) {
            try {
                $sql_icerik = file_get_contents($migration_yolu);
                ...
                log_ekle('islem', 'DB Migration uygulandi', 'v' . $dogru_surum . ': ' . $migration_sayisi . ' komut', $yonetici['id']);
            } catch (Throwable $e) {
